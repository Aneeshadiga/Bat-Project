...
class mMPLR:
    ...
    def getBatchesFromFile(self, destinationId, filepath="input.txt", isEncrypted=False, compress=False):
        data = self.B64Adapter.getStringFromFile(inputFile=filepath, compress=compress)
        self.setServiceType(self.B64Adapter.getDataType())
        return self.getPacketsAsBatches(data=data, dataType=self.getServiceType(), 
                destinationId=destinationId, isEncrypted=isEncrypted)

    def parseData(self, data, dataType=0, isBatches=False, isRaw=False, isEncrypted=True, writeToFile=False, outputFile="output"):
        message = self.parsePacketsAsBatches(data, isRaw=isRaw, isEncrypted=isEncrypted) if isBatches else self.parsePackets(data,isRaw=isRaw, isEncrypted=isEncrypted)
        if writeToFile:
            self.B64Adapter.writeStringToFile(input=message, outFile=outputFile, dataType=dataType)
            print("Written to ", self.B64Adapter.getOutputPath(), "Successfully")
            return 1
        else:
            return message
